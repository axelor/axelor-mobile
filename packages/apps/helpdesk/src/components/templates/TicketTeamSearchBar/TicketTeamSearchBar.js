...
import {fetchTeamTickets} from '../../../features/ticketSlice';

const TicketTeamSearchBar = ({
  ...
  const fetchTicketSearchBarAPI = useCallback(
    ({page = 0, searchValue}) => {
      dispatch(
        fetchTeamTickets({page: page, searchValue: searchValue, user: user}),
      );
    },
    [dispatch, user],
  );
...
export default TicketTeamSearchBar;
