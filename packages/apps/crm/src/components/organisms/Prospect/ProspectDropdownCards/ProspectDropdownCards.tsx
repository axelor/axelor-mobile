...
const ProspectDropdownCards = ({}) => {
  ...
  const refreshContactInfos = useCallback(() => {
    dispatch((fetchProspectById as any)({partnerId: prospect?.id}));
    dispatch((fetchPartnerAddresses as any)({partnerId: prospect?.id}));
  }, [dispatch, prospect?.id]);

  useEffect(() => {
    if (prospect.contactPartnerSet?.length > 0) {
      dispatch(
        searchContactById(prospect.contactPartnerSet?.map((_i: any) => _i.id)),
      );
    }
