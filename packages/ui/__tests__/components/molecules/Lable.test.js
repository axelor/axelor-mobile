/*
 * Axelor Business Solutions
 *
 * Copyright (C) 2023 Axelor (<http://axelor.com>).
 *
 * This program is free software: you can redistribute it and/or  modify
 * it under the terms of the GNU Affero General Public License, version 3,
 * as published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

import React from 'react';
import {shallow} from 'enzyme';
import {Label, Text, lightTheme} from '@axelor/aos-mobile-ui';
import {hexToRgb} from '../../../lib/utils/commons-utlis';
import {View} from 'react-native';

describe('Label Component', () => {
  const Colors = lightTheme.colors;
  const message = 'This is lable message.';

  it('should render without crashing', () => {
    const wrapper = shallow(<Label message={message} />);
    expect(wrapper.exists()).toBe(true);
  });

  it('should render with the correct message', () => {
    const wrapper = shallow(<Label message={message} />);
    expect(wrapper.find(Text).prop('children')).toBe(message);
  });

  it('should apply default "error" styles when type is not provided', () => {
    const wrapper = shallow(<Label message={message} />);

    expect(wrapper.find(View).prop('style')).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          backgroundColor: `rgba(${hexToRgb(
            Colors.errorColor.background_light,
          )}, 0.4)`,
          borderColor: Colors.errorColor.background_light,
          borderWidth: 1,
        }),
      ]),
    );
  });

  it('should apply "success" styles when type is "success"', () => {
    const wrapper = shallow(<Label message={message} type="success" />);

    expect(wrapper.find(View).prop('style')).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          backgroundColor: `rgba(${hexToRgb(
            Colors.successColor.background_light,
          )}, 0.4)`,
          borderColor: Colors.successColor.background_light,
          borderWidth: 1,
        }),
      ]),
    );
  });

  it('should apply "info" styles when type is "info"', () => {
    const wrapper = shallow(<Label message={message} type="info" />);

    expect(wrapper.find(View).prop('style')).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          backgroundColor: `rgba(${hexToRgb(
            Colors.infoColor.background_light,
          )}, 0.4)`,
          borderColor: Colors.infoColor.background_light,
          borderWidth: 1,
        }),
      ]),
    );
  });

  it('should apply "danger" styles when type is "danger"', () => {
    const wrapper = shallow(<Label message={message} type="danger" />);

    expect(wrapper.find(View).prop('style')).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          backgroundColor: `rgba(${hexToRgb(
            Colors.cautionColor.background_light,
          )}, 0.4)`,
          borderColor: Colors.cautionColor.background_light,
          borderWidth: 1,
        }),
      ]),
    );
  });

  it('should render a close icon when showClose is true', () => {
    const wrapper = shallow(<Label message={message} showClose />);

    expect(wrapper.find('Icon[name="times"]').exists()).toBe(true);
  });

  it('should call onClose when close icon is pressed', () => {
    const onCloseMock = jest.fn();
    const wrapper = shallow(
      <Label message={message} showClose onClose={onCloseMock} />,
    );

    wrapper.find('Icon[name="times"]').simulate('press');
    expect(onCloseMock).toHaveBeenCalled();
  });
});
