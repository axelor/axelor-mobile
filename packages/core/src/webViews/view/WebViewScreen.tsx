...
import {createWebViewActionID, createWebViewPath} from '../display.helpers';
import {webViewProvider, WebViewConfig} from '../webView.provider';
...
export const WebViewScreen = ({
  ...
  const [webViewData, setWebViewData] = useState<any>(
    defaultConfig ?? webViewProvider.getConfig(webViewId) ?? {},
  );
  ...
  const refresh = useCallback(() => {
    ...
    fetchWebViewById({mobileWebViewId: webViewId})
      .then(response => {
        ...
        if (config != null) {
          setWebViewData(config);
          webViewProvider.updateConfig(config);
        }
      })
      .catch(() => {})
  ...
  const hasConfig = useMemo(
    () => Object.keys(webViewData ?? {}).length > 0,
    [webViewData],
  );
  ...
  return (
    <WebView baseUrl={webViewData?.url} path={createWebViewPath(webViewData)} />
  );
};
